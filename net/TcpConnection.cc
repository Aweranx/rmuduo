      ...
      highWaterMark_(64 * 1024 * 1024)  // 64M
{
  ...
  LOG_INFO("TcpConnection::ctor[{}] at fd={}", name.c_str(), sockfd);
  socket_->setKeepAlive(true);
}

TcpConnection::~TcpConnection() {
  LOG_INFO("TcpConnection::dtor[{}] at fd={}, state={}", name_, channel_->fd(),
           (int)state_);
}
...
void TcpConnection::handleWrite() {
  if (channel_->isWriting()) {
    ...
    if (n > 0) {
      ...
      // buffer写空，先调用写完成回调再shutdownWrite
    ...
    LOG_ERROR("Connection fd={} is down, no more writing ", channel_->fd());
  }
}
// poller =>
// channel::closeCallback=>TcpConnection::handleClose=>TcpServer::removeConnection
void TcpConnection::handleClose() {
  LOG_INFO("TcpConnection::handleClose fd={} state={}", channel_->fd(),
           (int)state_);
  ...
  // connectioncallback里对state进行判断来执行不同的操作
...
void TcpConnection::handleError() {
  ...
  LOG_ERROR("TcpConnection::handleError name:{} - SO_ERROR:{} ", name_.c_str(),
            err);
}
...
/*
发送数据  应用写的快， 而内核发送数据慢，
需要把待发送数据放入缓冲区，设置水位回调
buffer为空就直接调用write，如果没发完将剩下的存入buffer
如果buffer有数据，就将data加入buffer， 注册epollout事件由handlewrite来处理
*/
void TcpConnection::sendInLoop(const void* data, size_t len) {
  ...
  // 也就是调用TcpConnection::handleWrite()方法， 把发送缓冲区中的数据发送完成
  if (!faultError && remaining > 0) {
    ...
    if (!channel_->isWriting()) {
      channel_
          ->enableWriting();  // 这里一定要注册channel的写事件，否则poller不会通知Epollout
    }
  }
}
