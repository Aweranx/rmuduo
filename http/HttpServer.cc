...
static std::string BADREQUEST = "HTTP/1.1 400 Bad Request\r\n\r\n";
...
HttpServer::HttpServer(EventLoop* loop, const InetAddress& listenAddr,
                       const std::string& nameArg, TcpServer::Option option)
...
void HttpServer::start() {
  ...
  LOG_INFO("HttpServer {} starts listening on {}", server_.name(),
           server_.ipPort());
  server_.start();
}
